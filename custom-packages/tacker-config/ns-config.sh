...
echo "[INFO] Creating HTTP client"
openstack server create --flavor user --image ubuntu --nic net-id=$network_id --user-data cloud-init.cfg http_client
echo "[INFO] Creating HTTP server"
openstack server create --flavor user --image ubuntu --nic net-id=$network_id --user-data cloud-init.cfg http_server
...
cat > ../vnffg-nsd/ns_param.yaml << EOL
nsd:
  vl1_name: mgmt
  vl2_name: net0
